...
func NewRouter(db *database.PostgresDB, cfg *config.Config) *gin.Engine {
	...
	v1 := router.Group("/api/v1")
	{
		...
		v1.GET("/products/:id/images/:imageId/content", handler.GetProductImageContent)
		...
		admin.Use(adminPrivateMiddleware())
		{
			...
			protected.Use(adminAuthMiddleware(cfg.Admin.Token))
			{
				...
				protected.DELETE("/products/:id", handler.DeleteAdminProduct)
				protected.PATCH("/products/:id/status", handler.UpdateAdminProductStatus)
				protected.GET("/products/:id/images", handler.ListProductImages)
				protected.POST("/products/:id/images", handler.UploadProductImages)
				protected.PATCH("/products/:id/images/order", handler.ReorderProductImages)
				protected.DELETE("/products/:id/images/:imageId", handler.DeleteProductImage)
